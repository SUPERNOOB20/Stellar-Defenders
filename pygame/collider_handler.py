...
def finds_line_equation(vertex_1: Vertex, vertex_2: Vertex):      # Extends the given line to fit the whole screen by finding its closed formula! So basically finds a and b so that line = a * x + b. Some odd dudes call it mx + b. Pay those no mind :p
    ...
    if vertex_1.x_coordinate() == vertex_2.x_coordinate():
        line_formula = Line(0, vertex_1.x_coordinate())                    # it's a horizontal line!

    elif vertex_1.y_coordinate() == vertex_2.y_coordinate():
        line_formula = Line("vertical", vertex_1.y_coordinate())     # it's a vertical line... let's make a flag for it to separate this border case from the rest ":3

    else:
        a = (vertex_2.y_coordinate() - vertex_1.y_coordinate()) / (vertex_2.y_coordinate() - vertex_1.x_coordinate())
...
def check_colliders_init(triangle: Triangle, vertex_A: Vertex):   # Looks for the (bool, bool, bool) combination for vA
    ...
    for line in lines:
        ...
        if type(line.slope()) == str:     # if the line is a vertical one...
            line = (line.ordinates() < vertex_A.y_coordinate())
...
def check_colliders(triangle: Triangle, vertex_AC: Vertex, vertex_C: Vertex):   # vertex_AC is the (bool, bool, bool) combination for vertex_A
